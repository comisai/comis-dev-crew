...
catalog changes. The service-side client exposes handshake, health, exact group-host rollup
reads, report, evidence, attention-response receive, exact approval-receipt consumption, and
workspace release. Generated activate, abandon, and terminal-event DTOs are inbound handler
contracts and cannot be used as outbound client methods. Strict runtime validation rejects
unknown or duplicate fields, trailing JSON, invalid closed discriminators, operation-envelope
disagreement, response identity drift, and size-limit violations before they can cross the
adapter boundary.
