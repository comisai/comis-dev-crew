...
func TestManifestRejectsIncompleteIdentityLimitsAndCatalogEntries(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nonpositive group limit", mutate: func(manifest *Manifest) { manifest.Limits.MaxGroupMembers = 0 }},
		{name: "inconsistent group limit", mutate: func(manifest *Manifest) { manifest.Limits.MaxGroupMembers++ }},
...
func TestManifestAcceptsAttentionResponseServiceScope(t *testing.T) {
	root, _ := writeFixtureBundle(t)
	verified, err := Open(root)
	if err != nil {
		t.Fatalf("open fixture bundle: %v", err)
	...
	manifest := cloneManifest(verified.Manifest)
	scope := "attention_response"
	manifest.MethodCatalog[0].RequiredServiceScope = &scope
	if err := validateManifest(manifest); err != nil {
		t.Fatalf("validateManifest(attention response scope) error = %v", err)
	}
}
...
func writeFixtureBundle(t *testing.T) (string, string) {
  ...
  "limits": {"maxEvidenceBytes":1048576,"maxGroupMembers":16,"maxInFlightRequests":32,"maxLineBytes":65536,"maxReportBytes":16384,"maxRequestBytes":65536,"maxResponseBytes":65536,"reportRetentionDays":30},
