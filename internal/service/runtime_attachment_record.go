...
func persistPinnedRuntimeAttachmentIdentity(
	...
	_, err = publishPinnedRuntimeAttachmentIdentity(pinned, record, pinned.taskIdentity, beforePublish)
	return err
}
...
func publishPinnedRuntimeAttachmentIdentity(
	...
	if existingFound && existingRecord.Task != priorTaskIdentity {
...
func parseRuntimeAttachmentIdentityRecord(encoded string) (runtimeAttachmentIdentityRecord, error) {
	if len(encoded) != 204 || encoded[len(encoded)-1] != '\n' {
	...
	record := runtimeAttachmentIdentityRecord{
		Task: reporter.RuntimeSocketIdentity{
			...
			BirthSec: int64(values[4]), BirthNsec: int64(values[5]),
		},
		Socket: reporter.RuntimeSocketIdentity{
			Device: values[6], Inode: values[7], ChangeSec: int64(values[8]), ChangeNsec: int64(values[9]),
			BirthSec: int64(values[10]), BirthNsec: int64(values[11]),
		},
	}
...
func readRuntimeAttachmentIdentityRecord(
	...
	encoded, readErr := io.ReadAll(io.LimitReader(file, 205))
	closeErr := file.Close()
	if statErr != nil || recordIdentityErr != nil || stat.Mode&unix.S_IFMT != unix.S_IFREG || stat.Mode&0o777 != 0o600 ||
		stat.Nlink != 1 || readErr != nil || len(encoded) > 204 || closeErr != nil {
