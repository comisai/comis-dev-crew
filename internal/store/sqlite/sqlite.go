...
const activatedAttachmentMigration = `
ALTER TABLE tasks ADD COLUMN execution_attachment_id TEXT NOT NULL DEFAULT '';
ALTER TABLE tasks ADD COLUMN attachment_target_name TEXT NOT NULL DEFAULT '';
INSERT OR IGNORE INTO schema_migrations(version, applied_at)
...
func (store *Store) migrate(ctx context.Context) error {
	...
	if err := store.applyVersionedMigration(ctx, 12, validationProcessMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 13, candidateEvidenceMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 14, comisEvidenceOutboxMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 15, taskHandbackMigration); err != nil {
		return err
	}
	...
	if err := store.applyVersionedMigration(ctx, 22, runtimeAttachmentRecoveryRefusalMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 23, taskPauseRequestMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 24, scoutPromotionMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 25, taskReplacementMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 26, taskSteeringMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 27, taskDiscardMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 28, scoutAttestationMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 29, decisionSurfacingMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 30, serviceEventMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 31, decisionCancellationMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 32, decisionResponseMigration); err != nil {
		return err
	}
	...
	if err := store.applyVersionedMigration(ctx, 35, initiativePreparationMigration); err != nil {
		return err
	}
	if err := store.applyVersionedMigration(ctx, 36, initiativeAbandonmentMigration); err != nil {
		return err
	}
	return store.applyVersionedMigration(ctx, 37, initiativeControlMigration)
}
