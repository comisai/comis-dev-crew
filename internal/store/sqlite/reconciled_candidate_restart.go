...
func resumableCandidateDelivery(
	ctx context.Context,
	transaction *sql.Tx,
	task domain.Task,
	at time.Time,
) (bool, error) {
	reconciled, err := resumableReconciledCandidateDelivery(ctx, transaction, task, at)
	if err != nil || reconciled {
		return reconciled, err
	}
	return resumableWorkerReportedCandidateDelivery(ctx, transaction, task, at)
}
...
func resumableWorkerReportedCandidateDelivery(
	ctx context.Context,
	transaction *sql.Tx,
	task domain.Task,
	at time.Time,
	...
	var candidateReports, deliveredReports int
	if err := transaction.QueryRowContext(ctx, `SELECT COUNT(*), COUNT(o.delivered_at)
		...
		&candidateReports, &deliveredReports,
	); err != nil {
		return false, fmt.Errorf("inspect restart worker candidate reports: %w", err)
	}
	var reconciliations int
	if err := transaction.QueryRowContext(ctx, `SELECT COUNT(*)
		FROM task_candidate_reconciliations WHERE task_handle = ?`, task.Handle).Scan(&reconciliations); err != nil {
		return false, fmt.Errorf("inspect restart worker candidate reconciliations: %w", err)
	}
	if candidateReports != 1 || deliveredReports != 0 || reconciliations != 0 {
		return false, nil
	}
	const evidenceQuery = `SELECT task_handle, evidence_digest, canonical,
		required_local_checks_json, required_forge_checks_json,
	...
	if err != nil {
		return false, fmt.Errorf("read restart worker candidate evidence: %w", err)
	}
	sealed, err := domain.ParseDeliveryEvidence(row.canonical, row.digest)
	if err != nil {
		return false, errors.New("read restart worker candidate evidence: stored evidence is invalid")
	}
	bundle := sealed.Bundle()
	if row.judgment.Outcome != domain.CandidateAccepted || row.stateVersion != task.StateVersion ||
		bundle.ProducedAt.After(at) || !bundle.ExpiresAt.After(at) ||
	...
	if err := transaction.QueryRowContext(ctx, `SELECT COUNT(*),
		...
		return false, fmt.Errorf("inspect restart worker candidate publications: %w", err)
	}
	return total == 2 && exact == 2, nil
}
