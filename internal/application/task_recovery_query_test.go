...
func TestQueries_ExplainUnknownTaskUsesReasonSpecificRecoveryEvidence(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "runtime relay identity is unproven", task: unknown, host: queryHostStatus(false),
			recovery: TaskRecoveryEvidence{Kind: RecoveryRuntimeRelayIdentityUnproven},
			wantCode: "runtime_relay_identity_unproven", wantAction: ActionInspectTask},
